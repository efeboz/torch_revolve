...
ActionKind = Literal["forward_store", "forward_drop", "restore", "backward"]
...
class Schedule:
    ...
    def validate(self, chain: ChainProfile) -> LegalityReport:
        ...
        for action_index, action in enumerate(self.actions):
            unit = action.unit
            if unit >= n_units:
                violations.append(f"action {action_index}: unit {unit} is outside the chain")
                continue
            if action.kind.startswith("forward"):
                ...
                if unit not in snapshots:
                    violations.append(
                        f"action {action_index}: state {unit} has no live snapshot"
                    )
                position = unit
                live_tape = None
                ...
                if position != unit + 1 or live_tape != unit:
                    violations.append(
            ...
            peak_bytes = max(peak_bytes, snapshot_bytes)

        if backward_expected != -1:
            violations.append(f"schedule omitted {backward_expected + 1} backward actions")
    ...
    def _state_bytes(chain: ChainProfile, state: int) -> int:
        return chain.units[state].activation_bytes
